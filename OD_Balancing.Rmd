...
```{r Check datasets}
    ...
    volume <- vol_3[[1]]$InputVol
    target <- tar_3[[1]]$Value
    percent  <- percent_3
  ...
  for(n in c(1:row_no)){
    ...
    repeat {
      ...
      repeat {
        ...
        if(!is.na(y[min_col]) & y[min_col] >= 0.01 & y[min_col] <= 0.99 | y[min_col] ==1){
          break
        }
      }
      ...
      repeat {
        ...
        if(!is.na(y[max_col]) & y[max_col] >= 0.01 & y[max_col] <= 0.99 | y[max_col] ==1){
          break
        }
      }
