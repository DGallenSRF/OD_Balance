...
# percent <- matrix(c(.2689,0,0,0,0,
#               .0936,.07516,0,0,0,
#               .3237,.4681,.49745,.54248,0,
...
sc
colSums(apply(sc, 2, function(x) x * volume))-target
```
...
final_data
rowSums(final_data)
colSums(apply(final_data,2,function(x) x*volume))-target
```
...
round(final_data - percent,3)


```

```{r}
heatmap(round(final_data - percent,3),Rowv = NA,Colv = NA,reorderfun = function(d,w) reorder() scale = 'row',col = heat.colors(256))
```
